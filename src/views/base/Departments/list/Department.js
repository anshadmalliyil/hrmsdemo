...
const Department = () => {
  ...
  const SaveCountry = ()=>{
    ...
    //if (CountryData.DEPARTMENT_NAME !="" && CountryData.DEPARTMENT_CODE != null){
      const url=baseURL+"api/Departments"+(CountryData.PK_DEPARTMENT_ID > 0 ? "/"+CountryData.PK_DEPARTMENT_ID+"":"");
 ...
 //For save notification
  ...
  const DeleteConfirm = (id)=>{
   ...
   //Modal popup for Edit
  const DeleteCountry = ()=>{
    const url=baseURL+"api/Departments/"+CountryData.PK_DEPARTMENT_ID+"";
 ...
 //For delete notification
  ...
  return (
    <>
      ...
      // onClick={toggle}
        // {...rest} 
        href={"/#/base/departments/create/"+item.PK_DEPARTMENT_ID} 
        rel="noreferrer noopener" 
        target="_blank" 
        className="card-header-action"  
      >
